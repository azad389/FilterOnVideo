...
class VideoFilterCompositor : NSObject, AVVideoCompositing{
    ...
    let renderQueue = DispatchQueue(label: "com.azad.videofilterexporter.renderingqueue", attributes: [])
    let renderContextQueue = DispatchQueue(label: "com.azad.videofilterexporter.rendercontextqueue", attributes: [])
        ...
        autoreleasepool(){
            self.renderQueue.sync{
                guard let instruction = request.videoCompositionInstruction as? VideoFilterCompositionInstruction else{
                    request.finish(with: NSError(domain: "azad.com", code: 760, userInfo: nil))
                    return
                }
                guard let pixels = request.sourceFrame(byTrackID: instruction.trackID) else{
                    request.finish(with: NSError(domain: "azad.com", code: 761, userInfo: nil))
                    return
                }
